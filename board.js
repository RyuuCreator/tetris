class Board {
    ...
    getEmptyBoard() {
        return Array.from({ length: wellRows }, () =>
            Array(wellColumns).fill(0)
        );
    }
    ...
    isLineComplete() {
        ...
        if (lines.length > 0) {
            ...
            if (lines.length == 4) {
                score.textContent =
                    parseInt(score.textContent) + fixedScores[4];
                tetrisCount++;
                if (
                    tetrisCount == levelTargets.level1.tcount &&
                    currentLevel == 1
                ) {
                    gameSpeed = levels[1];
                    levelElement.textContent =
                        parseInt(levelElement.textContent) + 1;
                    currentLevel++;
                    levelIncreased = true;
                }
                if (
                    tetrisCount == levelTargets.level2.tcount &&
                    currentLevel == 2
                ) {
                    gameSpeed = levels[2];
                    levelElement.textContent =
                ...
                if (
                    tetrisCount == levelTargets.level3.tcount &&
                    currentLevel == 3
                ) {
                    gameSpeed = levels[3];
                    levelElement.textContent =
